...
OUT_SM = zeros(length(xB), length(PdB));
for k = 1 : length(xB)
    fprintf('Running %d per %d \n',k,length(xB));
for aa = 1:length(PdB)
    OUT_SM(k,aa) = SPfunc(PdB(aa),IdB,LL,KK,PL,RR,xB(k),yB,xE,yE,eta,alpha,Num_Trial,kappa) ;
end
end
OUT_SM
h_out = semilogy(xB,OUT_SM,'bs'); grid on;hold on;;
end
