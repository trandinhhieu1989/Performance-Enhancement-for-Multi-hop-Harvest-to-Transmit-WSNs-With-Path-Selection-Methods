...
% This figure, we investigate the influences of of energy harvesting ratio alpha on OP
...
for aa = 1:length(PdB)
    OUT_SM(k,aa) = SPfunc(PdB(aa),IdB,LL,KK,PL,RR,xB,yB,xE,yE,eta,alpha(k),Num_Trial,kappa) ;
end
end
OUT_SM
h_out = semilogy(alpha,OUT_SM,'bs'); grid on;hold on;;
end
