function h_out = RP_COOP_SIM(PdB,IdB,LL,KK,PL,RR,xB,yB,xE,yE,eta,alpha,Num_Trial,kappa)
...
for aa = 1:length(PdB)
    OUT_SM(k,aa) = RPfunc(PdB(aa),IdB,LL,KK,PL,RR,xB,yB,xE,yE,eta,alpha,Num_Trial,kappa(k)) ;
end
end
OUT_SM
h_out = semilogy(kappa,OUT_SM,'g*'); grid on;hold on;
