function h_out = BP_COOP_SIM(PdB,IdB,LL,KK,PL,RR,xB,yB,xE,yE,eta,alpha,Num_Trial,kappa)
...
% From dB to Watt
OUT_SM = zeros(length(kappa), length(PdB));
for k = 1 : length(kappa)
    fprintf('Running %d per %d \n',k,length(kappa));
for aa = 1:length(PdB)
    OUT_SM(k,aa) = BPfunc(PdB(aa),IdB,LL,KK,PL,RR,xB,yB,xE,yE,eta,alpha,Num_Trial,kappa(k)) ;
end
end
OUT_SM
h_out = semilogy(kappa,OUT_SM,'ro'); grid on;hold on;
end
